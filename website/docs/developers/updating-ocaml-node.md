...
node references in the Rust node. The OCaml node releases are used to verify
that the current Rust node version maintains compatibility with the official
Mina Protocol implementation through end-to-end and scenario testing. This guide
walks through the process based on the workflow used in
...
The Rust node provides an automation script to handle the bulk of the update
process:
...
4. **End-to-End Testing**: Run scenario tests to verify Rust node compatibility
